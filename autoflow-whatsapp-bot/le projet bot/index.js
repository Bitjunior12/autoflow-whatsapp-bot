...
const MIN_POUSSINS_QTE = Number.parseInt(process.env.MIN_POUSSINS_QTE || "50", 10);
const MAX_POUSSINS_QTE = Number.parseInt(process.env.MAX_POUSSINS_QTE || "10000", 10);
const MIN_DEVIS_SUJETS = Number.parseInt(process.env.MIN_DEVIS_SUJETS || "100", 10);
const MAX_DEVIS_SUJETS = Number.parseInt(process.env.MAX_DEVIS_SUJETS || "50000", 10);
const ADMIN_API_KEY = process.env.ADMIN_API_KEY || "";
const PROTECT_ADMIN_ROUTES = process.env.PROTECT_ADMIN_ROUTES !== "false";
const recentIncomingMessages = new Map();

connectDB();

function requireAdminKey(req, res, next) {
  if (!PROTECT_ADMIN_ROUTES) return next();
  if (!ADMIN_API_KEY) {
    return res.status(503).json({
      error: "ADMIN_API_KEY manquante côté serveur",
    });
  }
...
function cleanupRecentMessages() {
  const now = Date.now();
  for (const [id, timestamp] of recentIncomingMessages.entries()) {
    if (now - timestamp > 10 * 60 * 1000) {
      recentIncomingMessages.delete(id);
    }
  }
}
...
async function safeSend(to, message) {
  try {
    ...
    console.error(`❌ Erreur envoi WhatsApp vers ${to} :`, err.message);
    return false;
  }
}
...
async function handleMessage(from, text) {
  ...
  const ADMIN_PHONE = process.env.ADMIN_PHONE || "22502642080"; // TON numéro
...
⏳ Merci de réessayer dans quelques heures.`;
  ...
  const isInCriticalFlow = [
    "choix_race", "quantite", "nom", "devis_choix", "devis_nom",
...
↩️ Tapez *menu* pour revenir au menu principal`;
  ...
  if (session?.step === "choix_race") {
    if (!PRIX_POUSSINS[msg]) {
      return `❌ Choix invalide.\n\n${MENU_RACES}`;
    }
    const selected = PRIX_POUSSINS[msg];
    await setSession(from, {
      step: "quantite",
      race: selected.race,
      unitPrice: selected.prix,
    });
    return `✅ Race choisie : *${selected.race}*\n💰 Prix unitaire : *${selected.prix.toLocaleString("fr-FR")} FCFA*\n\n📦 *Combien de poussins souhaitez-vous commander ?*\n_(Entrez un nombre, ex: 500)_`;
  }

  if (session?.step === "quantite") {
    const quantity = Number.parseInt(msg, 10);
    if (!Number.isInteger(quantity) || quantity <= 0) {
      return `❌ Quantité invalide. Entrez un nombre supérieur à 0.`;
    }
    if (quantity < MIN_POUSSINS_QTE || quantity > MAX_POUSSINS_QTE) {
      return `❌ Quantité hors limite.\n\nVeuillez entrer une quantité entre *${MIN_POUSSINS_QTE}* et *${MAX_POUSSINS_QTE}* poussins.`;
    }
    if (!session?.race || !session?.unitPrice) {
      await clearSession(from);
      return `⚠️ Session expirée. Reprenez la commande.\n\n${MENU_RACES}`;
    }
    await setSession(from, { ...session, step: "nom", quantity });
    const total = quantity * session.unitPrice;
    return `📦 Quantité enregistrée : *${quantity}*\n💰 Total estimé : *${total.toLocaleString("fr-FR")} FCFA*\n\n👤 *Quel est votre nom complet ?*`;
  }

  if (session?.step === "nom") {
    const name = text.trim();
    if (name.length < 2) return `❌ Nom invalide. Entrez votre nom complet.`;
    if (!session?.race || !session?.unitPrice || !session?.quantity) {
      await clearSession(from);
      return `⚠️ Session expirée. Reprenez la commande.\n\n${MENU_RACES}`;
    }
    try {
      const totalPrice = session.quantity * session.unitPrice;
      await Order.create({
        phone: from,
        name,
        race: session.race,
...
📞 Un conseiller vous contactera sous *24h* pour la confirmation.

↩️ Tapez *menu* pour revenir au menu principal`;
    } catch (err) {
      console.error("❌ Erreur enregistrement commande :", err.message);
      return `❌ Une erreur est survenue pendant l'enregistrement de votre commande. Tapez *menu* pour recommencer.`;
    }
  }
  ...
  if (session?.step === "devis_choix") {
    if (msg === "1") {
      await setSession(from, { step: "devis_nom", profil: "devis_batiment" });
      return DEVIS_BATIMENT_INFO;
    }
    if (msg === "2") {
      await setSession(from, { step: "devis_nom", profil: "devis_complet" });
  ...
  if (session?.step === "devis_nom") {
    const name = text.trim();
    if (name.length < 2) return `❌ Nom invalide. Entrez votre nom complet.`;
    await setSession(from, { ...session, step: "devis_ville", name });
    return `👤 Nom enregistré : *${name}*\n\n📍 *Dans quelle ville se trouve votre projet ?*`;
  }

  if (session?.step === "devis_ville") {
    const ville = text.trim();
    if (ville.length < 2) return `❌ Ville invalide.`;
    await setSession(from, { ...session, step: "devis_superficie", ville });
    return `📍 Ville enregistrée : *${ville}*\n\n📐 *Quelle est la superficie du terrain (en m²) ?*`;
  }

  if (session?.step === "devis_superficie") {
    const superficie = text.trim();
    if (superficie.length < 1) return `❌ Superficie invalide.`;
    await setSession(from, { ...session, step: "devis_sujets", superficie });
    return `📐 Superficie enregistrée : *${superficie}*\n\n🐥 *Combien de poussins prévoyez-vous ?* _(nombre)_`;
  }

  if (session?.step === "devis_sujets") {
    const sujets = Number.parseInt(msg, 10);
    if (!Number.isInteger(sujets) || sujets <= 0) {
      return `❌ Nombre de poussins invalide. Entrez un nombre supérieur à 0.`;
    }
    if (sujets < MIN_DEVIS_SUJETS || sujets > MAX_DEVIS_SUJETS) {
      return `❌ Nombre de sujets hors limite.\n\nVeuillez entrer une valeur entre *${MIN_DEVIS_SUJETS}* et *${MAX_DEVIS_SUJETS}* sujets.`;
    }
    if (!session?.name || !session?.ville || !session?.superficie || !session?.profil) {
      await clearSession(from);
      return `⚠️ Session expirée. Reprenez votre demande.\n\n${MENU_DEVIS}`;
    }
    try {
      await Registration.create({
        phone: from,
      ...
      await Contact.findOneAndUpdate(
        { phone: from },
        { name: session.name, lastSeen: new Date() },
        { upsert: true, new: true, setDefaultsOnInsert: true }
      );
...
📍 Ville : ${session.ville}
📐 Superficie : ${session.superficie}
🐥 Nombre de poussins : ${sujets}
📦 Type : ${session.profil === "devis_complet" ? "Devis complet" : "Devis bâtiment"}
...
↩️ Tapez *menu* pour revenir au menu principal`;
      ...
      return `❌ Une erreur est survenue pendant l'enregistrement du devis. Tapez *menu* pour recommencer.`;
    }
  }
...
app.get("/debug-subscription", requireAdminKey, async (req, res) => {
  try {
    const wabaId = process.env.WABA_ID;
    const token = process.env.WHATSAPP_TOKEN;
...
app.get("/register-phone", requireAdminKey, async (req, res) => {
  try {
    const response = await fetch(
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    if (messageId) {
      ...
      if (recentIncomingMessages.has(messageId)) {
        console.log(`ℹ️ Message déjà traité (doublon) : ${messageId}`);
        return;
      }
      recentIncomingMessages.set(messageId, Date.now());
    }
    ...
    if (!text) {
      console.log("ℹ️ Message texte vide ignoré");
      return;
    }
    ...
    const sent = await safeSend(from, reponse);
    if (sent) {
...
app.post("/send-message", requireAdminKey, async (req, res) => {
...
app.post("/orders/:id/status", requireAdminKey, async (req, res) => {
  try {
    const { id } = req.params;
...
app.post("/registrations/:id/status", requireAdminKey, async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
    if (!["en_attente", "confirmée", "annulée"].includes(status)) {
...
app.get("/contacts", requireAdminKey, async (req, res) => {
  try {
...
app.get("/orders", requireAdminKey, async (req, res) => {
  try {
...
app.get("/registrations", requireAdminKey, async (req, res) => {
  try {
...
app.get("/devis", requireAdminKey, async (req, res) => {
  try {
...
app.get("/test-claude", requireAdminKey, async (req, res) => {
  try {
