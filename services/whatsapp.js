const sendWhatsAppMessage = async (to, message) => {
  ...
  if (!response.ok) throw new Error(`Erreur Meta: ${JSON.stringify(data.error)}`);
  return data;
};
...
const sendWhatsAppPDF = async (to, pdfBuffer, filename = 'devis.pdf', caption = '') => {
  ...
  if (!uploadResponse.ok || !uploadData.id) throw new Error(`Erreur upload PDF: ${JSON.stringify(uploadData)}`);
  const sendResponse = await fetch(`https://graph.facebook.com/v21.0/${phoneNumberId}/messages`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ messaging_product: 'whatsapp', recipient_type: 'individual', to, type: 'document', document: { id: uploadData.id, filename, caption } }),
  });
  const sendData = await sendResponse.json();
  if (!sendResponse.ok) throw new Error(`Erreur envoi PDF: ${JSON.stringify(sendData.error)}`);
  return sendData;
};
